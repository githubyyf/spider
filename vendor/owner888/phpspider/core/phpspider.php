...
class phpspider
{
    ...
    function __construct($configs = [])
    {
        ...
        $configs['client_ip'] = isset($configs['client_ip']) ? $configs['client_ip'] : [];
        ...
        self::$db_config = isset($configs['db_config']) ? $configs['db_config'] : [];
        self::$queue_config = isset($configs['queue_config']) ? $configs['queue_config'] : [];
    ...
    {
        return empty(self::$configs[$name]) ? [] : self::$configs[$name];
    }

    public function add_scan_url($url, $options = [], $allowed_repeat = true)
    {
        // 投递状态
        $status = false;
    ...
    public function install_signal()
    {
        if (function_exists('pcntl_signal')) {
            ...
            pcntl_signal(SIGINT, [&$this, 'signal_handler'], false);
            // status
            pcntl_signal(SIGUSR2, [&$this, 'signal_handler'], false);
            // ignore
        ...
        if ($http_code != 200) {
            ...
            if ($http_code == 301 || $http_code == 302) {
                ...
                if ($http_code == 407) {
                    ...
                    self::$collect_fail++;
                } elseif (in_array($http_code, ['0', '502', '503', '429'])) {
                    // 采集次数加一
                    $link['try_num']++;
                    // 抓取次数 小于 允许抓取失败次数
    ...
    public function check_cache()
    {
        ...
        if ($count != 0) {
            ...
            $arg = empty($arg) || !in_array($arg, ['y', 'n']) ? 'y' : $arg;
            if ($arg == 'n') {
                foreach ($keys as $key) {
     ...
     * @created time :2016-09-23 17:13
     */
    public function queue_lpush($link = [], $allowed_repeat = false)
    {
     ...
     * @created time :2016-09-23 17:13
     */
    public function queue_rpush($link = [], $allowed_repeat = false)
    {
        if (empty($link) || empty($link['url'])) {
            return false;
        }
